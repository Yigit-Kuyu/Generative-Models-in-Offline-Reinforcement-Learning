...
class EMA(): #  updates an Exponential Moving Average of network's parameters
    '''
...
class Trainer:
    ...
    def train_iteration(self, num_steps, iter_num=0, log_writer=None):
        """
        Trains the actor and critic models for one iteration, consisting of multiple steps.

        This function performs the following tasks:
        1. Trains the actor and critic models for a specified number of steps
        2. Updates learning rate schedulers if lr_decay is enabled
        3. Logs various loss metrics and training statistics
        4. Evaluates the trained models using provided evaluation functions
        5. Computes and logs the best return and normalized score achieved

        Args:
            num_steps (int): Number of training steps to perform in this iteration
            iter_num (int): Current iteration number (default: 0)
            log_writer (object): Logger object for writing logs (default: None)

        Returns:
            dict: A dictionary containing various logs and metrics from the training iteration
        """
        logs = dict()
        ...
        for key, value in loss_metric.items():
            print(f"{key.replace('_', ' ').title()}: {np.mean(value):.4f}")
        ...
        for eval_fn in self.eval_fns: 
            outputs = eval_fn(self.actor, self.critic_target)  # call --> eval_episodes(target_rew)--> evaluate_episode_rtg
        ...
        # Print iteration summary and compute best returns
        print('=' * 80)
        ...
        print(f'Current actor learning rate: {self.actor_optimizer.param_groups[0]["lr"]}')
        print(f'Current critic learning rate: {self.critic_optimizer.param_groups[0]["lr"]}')
    ...
    def train_step(self, log_writer=None, loss_metric={}):
        ...
        if self.max_q_backup: # False
            ...
            _, next_action, _ = self.ema_model(states_rpt, actions_rpt, rewards_rpt, None, rtg_rpt[:,:-1], timesteps_rpt, attention_mask=attention_mask_rpt)
        else: # True
            # The ema_model is essentially a copy of the actor network 
            # that is updated using an exponential moving average of the actor network's parameters. T
            # the actor network is an instance of the DecisionTransformer class.
            _, next_action, _ = self.ema_model(states, actions, rewards, action_target, rtg[:,:-1], timesteps, attention_mask=attention_mask)
        ...
        if self.k_rewards: # False
            ...
            else: # True, next_action comes from the ema_model
        ...
        # The critic loss is calculated as the mean squared error (MSE) between the current Q-values and the target Q-values. 
        # The attention mask is applied to ensure that only the valid (non-padded) elements are considered in the loss calculation.
        ...
        '''Policy Training'''  
        # The actor network is an instance of the DecisionTransformer class.
        # Forward method takes the input sequences and generates predictions.
        ...
        if reward_preds is not None: # False
            ...
            rewards_loss = 0
        bc_loss = F.mse_loss(action_preds_, action_target_) + states_loss + rewards_loss # behavior cloning loss
